...
P360D.ToolBar = function( textures, container, browser, accelerate, toggleLights, screenshot, toggleScenes, toggleMovie, toggleAudio, toggleCaption )
{
	...
	scope.buttons = [];//bar里面的按钮，为按钮增加监听器
	...
	for ( var i = 0, il = scope.textures.length - 6; i < il; i++ ) {
		...
		scope.buttons[ i ].addEventListener( browser.endEvent, onButtonsDown, false );//加入点击事件
	...
	this.enabledButtons = function( value )
	{
		...
		if ( !value ) {
			...
			try{
				...
				console.log("This is the first time rendering. There is no Child node in this node.")
			}
...
P360D.Earth = function( diffuse, bumpMap, specularMap, cloudsMap, nightMap, moonMap, stormMap )
{
	...
	earth.rotation.z = THREE.Math.degToRad( 23 );//黄赤交角
	scope.add( earth );

	// EARTH
	var earthMaterial = new P360D.EarthMaterial( diffuse, bumpMap, specularMap );
	var earthGeometry = new THREE.OctahedronGeometry( radius, segments );//集合体面数为8的5次方
	...
	var cloudsGeometry = new THREE.OctahedronGeometry( radius + 0.3, segments );//比地球大一点
...
//		var stormGeometry = new THREE.PlaneGeometry( P360D.Utils.randomNumber( 10, 30 ), P360D.Utils.randomNumber( 10, 30 ) );
		...
		// storms[ i ].lookAt( vector );//？？调整storm的方向？注销掉也没问题
	...
	nightGeometry.applyMatrix( new THREE.Matrix4().makeScale( -1, 1, 1 ) );//缩放矩阵
	var night = new THREE.Mesh( nightGeometry, nightMaterial );//backside
...
P360D.Earth.prototype.toggleLights = function( isNight )
{
	...
	if ( !isNight ) {
		...
		tween.resume();// 不是晚上就关灯

	} else {

		tween.pause();
		TweenLite.to( this.nightMaterial, 0.5, { opacity: 0.8 } );//晚上把灯打开
...
P360D.Earth.prototype.update = function( speed, camera ) 
{
	...
	if ( speed === 0 ) {
		...
		// console.log("the speed now is "+this.earth.rotation.y);
		// console.log("the speed now is "+this.earth.rotation.y);
...
//			transparent:true
	...
	var geometry = new THREE.BoxGeometry( d, d, d );//ddd是个什么鬼参数

	var material = new THREE.MeshFaceMaterial( materials );//同一个物体多个纹理贴图
...
//创建地球和银河
//还需要加入卫星
P360D.OrbitScene = function( skyTextures, earthTextures,data )
{
	THREE.Object3D.call( this );
	...
	//处理json数据，加入data
	scope.satellite=[];
	scope.satellite[0]=new P360D.Satellite(data.series[0].satellites[0]);
	scope.add(scope.satellite[0]);
	// scope.satellite[0].position.set(0,0,300);
...
P360D.OrbitScene.prototype.update = function( speed, camera )
{
	...
	this.satellite[0].update();
};

//获取数据
P360D.Data = function(path){
...
//卫星部分
P360D.Satellite= function(satelliteData){
	THREE.Object3D.call( this );//call调用函数，js知识，将函数中的this作为参数传给object3D
	/*
	调用包括方法调用、调用函数、构造器函数、call/apply函数
	参考资料：https://blog.csdn.net/sinat_35767703/article/details/68487735
	 */
	this.id=satelliteData.id;
	this.name=satelliteData.name;
	this.heigh=satelliteData.Heigh;
	this.period=satelliteData.Period;
	this.angle=satelliteData.angle;
	this.category=satelliteData.cateorgy;
	this.initDegree=0;//卫星初始位置，以角度表示，单位应该为rad
	var object=new THREE.Object3D();
	var scope=this;
	scope.add(object);
	var geometry=new THREE.OctahedronGeometry(20,8);//直径为20
	var objectMesh=new THREE.Mesh(geometry);
	object.add(objectMesh);
	scope.position.set(0,0,300);//替换成半径，需改
	...
	this.getInitDegree=function(){
		return ++this.initDegree>=360?this.initDegree-=360:this.initDegree;//需要调整速度
	}
	...
	this.setSatellitePosition=function(x,y,z){
		this.scope.position.set(x,y,z);//卫星位置
	}
...
P360D.Satellite.prototype = Object.create( THREE.Object3D.prototype );//prototype是创建对象实例……？
/*
prototype参考资料：https://www.cnblogs.com/socool-hu/p/5665270.html
*/

P360D.Satellite.prototype.update=function(){
	//加入每一帧刷新的函数
	var category=this.getCategory();
	var cid;
	var Rdegree=THREE.Math.degToRad(this.getInitDegree());
	// console.log(category);
	if(category==="太阳同步卫星") cid=1;
	else if(category==="静止卫星") cid=1;
	...
	switch(cid){
		...
		case 1:
		//静止轨道的计算
		this.setSatellitePosition(
			Math.sin(Rdegree)*300,
			0,//静止轨道这个值不变
			Math.cos(Rdegree)*300//cos rad*半径
		);
		break;
