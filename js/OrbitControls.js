...
THREE.OrbitControls = function ( object, domElement ) {
	...
	this.autoRotateDirection = 'right';
	...
	this.updateTheta = function( angle )
	{
		...
		{
			if( scope.autoRotateDirection == 'right' )
			{
				thetaDelta -= angle;
			}

			if( scope.autoRotateDirection == 'left' )
			{
				thetaDelta += angle;
			}
		}
...
*/		}
		...
		if( scope.autoRotateDirection == 'left' || !scope.autoRotate )
		{
			thetaDelta += angle;
		}
	}
	...
	this.updatePhi = function( angle, phi )
	{
		if( scope.enabledAutoRotatePhi && !isDown ){
			...
			if( phi < scope.maxPolarAngle-0.01 && dirPhi == 'down' )
			{
				phiDelta += ( angle * scope.phiRotationSpeed );
			}
			else if( phi > scope.maxPolarAngle-0.02 && dirPhi == 'down' )
			{
				dirPhi = 'up';
			}
			else if( phi > scope.minPolarAngle+0.01 && dirPhi == 'up' )
			{
				phiDelta -= ( angle * scope.phiRotationSpeed );
			}
			else if( phi < scope.minPolarAngle+0.02 && dirPhi == 'up' )
			{
				dirPhi = 'down';
			}
		}
...
//		console.log(this.noRotate +"  "+this.noZoom+"  "+this.enabled);
	}
