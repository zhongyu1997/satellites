...
function lateInit() {
	...
	//监听事件load，加载完毕调用onLoadImageLoaded
	...
	viewport = P360D.DOM.div( 'viewport' );//创建主要的dom
	viewport.style.position = 'absolute';

	scene = new THREE.Scene();//场景

	camera = new THREE.PerspectiveCamera( 40, VIEW_WIDTH / VIEW_HEIGHT, 1, 40000 );//相机
	cameraOrbit = new THREE.PerspectiveCamera( 40, VIEW_WIDTH / VIEW_HEIGHT, 1, 40000 );//又一个相机

	renderer = new THREE.WebGLRenderer( { antialias: true, alpha: true } );
	renderer.setSize( VIEW_WIDTH, VIEW_HEIGHT );
	renderer.setClearColor( 0x000000, 1 );
	renderer.sortObjects = false;
	viewport.appendChild( renderer.domElement );//加入渲染器画笔？

	controls = new THREE.OrbitControls( cameraOrbit, viewport );//场景的轨道控制器，平移缩放等
	cameraOrbit.position.set( 841, -429, -500 );//设置位置
	//设置轨道控制器的范围、速度等等
	...
	topCameraTake11.add( take11Cam );//也是object3D
	...
	take11Cam.rotation.y = THREE.Math.degToRad( 180 );//按照y轴寻转180

	var ambientLight = new THREE.AmbientLight( 0x07215c );//环境光，没有阴影
	ambientLight.color.setRGB( 0.02, 0.02, 0.07 );
	scene.add( ambientLight );

	var sunLight = new THREE.PointLight( 0xe8f7ff, 1.2 );//点光源
	var sunLight1 = new THREE.DirectionalLight( 0x000000, 0.0 );//平行光
	...
	document.body.appendChild( wrapper );//wrapper包括viewport和bar

	wrapper.appendChild( viewport );

	browser.disabledTouch();

	var endScreen = new P360D.EndAnimationScreen();//结束后成为endScreen
	...
	var imgLoader = new PxLoader();//声音图片预加载的库
	data=P360D.Data('../data/satellites.json');
	...
	function imageLoaderComplete( event ) {
		...
		//创建地球和银河
		scenes[ 0 ] = new P360D.OrbitScene( cubeTextures, earthTextures,data );
		animation.add( scenes[ 0 ] );

		earthOrbit = new P360D.OrbitScene( cubeTextures, earthTextures,data );
		orbit.add( earthOrbit );
		orbit.visible = false;//放入一个不可见的？？
		...
		for ( var i = 0, il = diffuseTextures.length; i < il; i++ ) {
			...
			animation.add( topScene );//？这是在干嘛，不停的新建地球的mesh？
		...
		toolBar.enabledButtons( isOrbiting );//false，没有切换到手动模式就不能使用toggle的按钮
...
//开始动画,需要改为开始orbit
	...
	function animateCamera() {
		...
		//addCallback，以回调的形式传参数，即为以函数作为参数
		timelineCamera.addCallback( function() {

			sceneTake = SceneTypes.TAKE_1;//从场景1开始
	...
	function startOrbiting() {
		...
		lastAnimationTake = sceneTake;//第一次赋值，因为首先就是先切到旋转
...
function render() 
{
	requestAnimationFrame( render );//递归，根据浏览器的帧数来渲染画面
